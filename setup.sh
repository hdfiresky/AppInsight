...
apt-get update -qq
apt-get install -y -qq openjdk-17-jdk unzip wget 2>&1 | tail -1
...
if [ "$ARCH" = "aarch64" ]; then
    ...
    # Register x86_64 binfmt (needed if not using wrapper scripts)
...
if [ ! -f "$SDK_DIR/cmdline-tools/latest/bin/sdkmanager" ]; then
    ...
    rm -f /tmp/cmdline-tools.zip /tmp/cmdline
...
echo -n "aapt2: "; "$SDK_DIR/tools/bin/aapt2" version 2>&1 || \
    "$SDK_DIR/build-tools/36.1.0/aapt2" version 2>&1
echo -n "d8: "; "$SDK_DIR/build-tools/36.1.0/d8" --version 2>&1 | head -1 || true
echo -n "apksigner: "; "$SDK_DIR/build-tools/36.1.0/apksigner" version 2>&1 | head -1 || true

echo ""
echo "=== Setup complete ==="
echo "Build: cd $(pwd) && ./build.sh"
echo "APK:   build/apk/app-debug.apk"
